...
$sql = "SELECT od.id,t.alias_id,od.order_date,total_price,num,foods,is_paid,waiter,od.comment,od.service_rate,od.table_name,od.total_price_present FROM `table` t LEFT OUTER JOIN 
...
foreach ($rs as $row){	
	...
	{			
		$on = "class='on'";
		$action = ' onclick="showOrderDetail(&quot;'.$row["id"].'&quot;,&quot;'.$row["alias_id"].'&quot;,&quot;'.$row["order_date"].'&quot;,&quot;'.$row["total_price"].'&quot;,&quot;'.$row["num"].'&quot;,&quot;'.$row["foods"].'&quot;,&quot;'.$row["is_paid"].'&quot;,&quot;'.$row["waiter"].'&quot;,&quot;'.$row["type_name"]."&quot;,&quot;".$row["totalPrice_2"].'&quot;,&quot;'.$row["category_name"].'&quot;,&quot;'.$row["comment"].'&quot;,'.$row["service_rate"].',&quot;'.$row["table_name"].'&quot;,'.$row["total_price_present"].')"';
	}
