...
if($editType == "editOrder")
{
	...
	if($table_id != "-")
	{
		...
		{
			echo "<script>alert('台号不存在，请输入其它台号！');editOrder('$id','$total_price_2','order.php','$type_value','$table_id','$sel_category');</script>";
			$validate = false;	
		}		
	}
	else
...
{	
	...
	$sql3 = "INSERT INTO 
			...
			`wireless_order_db`.`order_food`.`taste_id2` AS `taste_id2`,
			`wireless_order_db`.`order_food`.`taste_id3` AS `taste_id3`,
			`wireless_order_db`.`order_food`.`is_temporary` AS `is_temporary`,
