...
public class OrderFoodReflector {
	...
	public static OrderFood[] getDetailToday(DBCon dbCon, String extraCond,	String orderClause) throws SQLException {
		String sql;
//		sql = "SELECT C.order_id, C.food_alias, C.taste_alias, C.taste2_alias, C.taste3_alias, C.hang_status, C.is_temporary, "
//				+ " MAX(C.kitchen_alias) AS kitchen_alias, MAX(C.kitchen_id) AS kitchen_id, " 
//				+ " MAX(C.food_id) AS food_id, MAX(C.name) AS name, MAX(C.food_status) AS food_status, " 
//				+ " MAX(C.unit_price) AS unit_price, MAX(C.order_date) AS order_date,  MAX(C.discount) AS discount, "
//				+ " MAX(C.taste) AS taste, MAX(C.taste_price) AS taste_price, "
//				+ " MAX(C.taste_id) AS taste_id, MAX(C.taste2_id) AS taste2_id, MAX(C.taste3_id) AS taste3_id, " 
//				+ " MAX(C.dept_id) AS dept_id, MAX(C.id) AS id, SUM(C.order_count) AS order_sum, "
...
//				+ " GROUP BY C.order_id, C.food_alias, C.taste_alias, C.taste2_alias, C.taste3_alias, C.hang_status, C.is_temporary "
//				+ " HAVING order_sum > 0 " 
//				+ (orderClause == null ? "" : " " + orderClause);
		...
		while (dbCon.rs.next()) {
			...
			food.orderID = dbCon.rs.getLong("order_id");
...
//				+ (orderClause == null ? "" : " " + orderClause);
		...
		sql = "SELECT C.order_id, C.food_alias, C.taste_alias, C.taste2_alias, C.taste3_alias, C.is_temporary, "
				+ " MAX(C.kitchen_alias) AS kitchen_alias, MAX(C.kitchen_id) AS kitchen_id, " 
				+ " MAX(C.food_id) AS food_id, MAX(C.name) AS name, MAX(C.food_status) AS food_status, " 
				+ " MAX(C.unit_price) AS unit_price, MAX(C.order_date) AS order_date,  MAX(C.discount) AS discount, "
				+ " MAX(C.taste) AS taste, MAX(C.taste_price) AS taste_price, "
				...
				+ " WHERE C.order_id = B.id "
				+ (extraCond == null ? "" : extraCond)
				+ " GROUP BY C.order_id, C.food_alias, C.taste_alias, C.taste2_alias, C.taste3_alias, C.is_temporary "
				+ " HAVING order_sum > 0 " 
				+ (orderClause == null ? "" : " " + orderClause);
