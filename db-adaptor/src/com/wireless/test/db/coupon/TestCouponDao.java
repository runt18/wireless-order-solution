...
public class TestCouponDao {
	...
	public void tesCouponDao() throws SQLException, BusinessException{
		...
		try{
			...
			CouponType.InsertBuilder insertBuilder = new CouponType.InsertBuilder("测试优惠券类型", 30);//.setExpired(System.currentTimeMillis() / 1000 * 1000);
			...
			CouponDao.insertAll(mStaff, new Coupon.InsertAllBuilder(couponTypeId)
			  									  .addMemberId(members.get(0).getId())
			  									  .addMemberId(members.get(1).getId()));
			
			Assert.assertEquals("coupon amount by type after inserting two ones", 2, CouponDao.getByType(mStaff, couponTypeId).size());
			
			Assert.assertEquals("coupon amount by member after inserting one to member 1", 1, CouponDao.getByMember(mStaff, members.get(0).getId()).size());
			Coupon actualCoupon1 = CouponDao.getByMember(mStaff, members.get(0).getId()).get(0);
			...
			Assert.assertEquals("coupon amount by member after inserting one to member 2", 1, CouponDao.getByMember(mStaff, members.get(1).getId()).size());
			Coupon actualCoupon2 = CouponDao.getByMember(mStaff, members.get(1).getId()).get(0);
			...
			Assert.assertEquals("coupon amount by member after inserting one to member 1", 2, CouponDao.getByMember(mStaff, members.get(0).getId()).size());
			actualCoupon1 = CouponDao.getByMember(mStaff, members.get(0).getId()).get(1);
