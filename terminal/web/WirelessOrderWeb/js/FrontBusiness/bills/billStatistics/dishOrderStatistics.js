		...
		if (msg != "normal") {
			...
			menuStatResultGrid.getStore().each(function(record) {
				...
				for ( var i = 0; i < kitchenMultSelectData.length; i++) {
					if (record.get("kitchenAlias") == kitchenMultSelectData[i][0]) {
						record.set("kitchenDisplay", kitchenMultSelectData[i][1]);
					}
					if (record.get("kitchenAlias") == "SUM") {
						record.set("kitchenAlias", "");
					}
				}
				// 提交，去掉修改標記
				record.commit();
