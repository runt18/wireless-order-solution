...
var shiftCheckTableWin = new Ext.Window({
	...
	buttons : [ {
		text : "打印",
		handler : function() {
			var onDuty = shiftStatResultStore.getAt(shiftDtlRowIndex).get("beginTime");
			var offDuty = shiftStatResultStore.getAt(shiftDtlRowIndex).get("endTime");
	...
	Ext.Ajax.request({
		...
		success : function(response, options) {
			...
			if (rootData[0].message == "normal") {
				document.getElementById("shiftOperator").innerHTML = shiftStatResultStore.getAt(rowIndex).get("staff");
