...
var filterTypeComb = new Ext.form.ComboBox({
	...
	listeners : {
		select : function(combo, record, index) {
			...
			var typeSelectStore = new Ext.data.SimpleStore({
				fields : [ "value", "text" ],
				data : typeAddData
			});
			...
			} else if (conditionType == "typeComb") {
				searchForm.remove("typeComb");
			}
			...
			if (index == 0) {
				...
				// 狀態
				searchForm.add(typeComb);
				operatorComb.setValue(1);
				operatorComb.setDisabled(true);
				conditionType = "typeComb";
			}
					...
					.on(
							...
							function() {
								...
								if (conditionType == "text" && queryTpye != 0) {
									...
									queryValue = searchForm
											.findById("typeComb").getValue();
								}
								// else if (conditionType == "statusComb") {
								// queryValue = searchForm.findById(
								// "statusComb").getValue();
