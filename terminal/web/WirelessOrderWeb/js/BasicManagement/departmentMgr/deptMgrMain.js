...
Ext
		.onReady(function() {
			...
			deptGrid = new Ext.grid.EditorGridPanel({
				...
				tbar : [ {
					...
					handler : function() {
						...
						var modfiedArr = [];
						deptGrid.getStore().each(
								function(record) {
									if (record.isModified("deptName") == true) {
										modfiedArr.push(record.get("deptID")
												+ " field_separator "
												+ record.get("deptName"));
									}
								});
						...
						if (modfiedArr.length != 0) {
							...
							for ( var i = 0; i < modfiedArr.length; i++) {
								modDepts = modDepts + modfiedArr[i]
										+ " record_separator ";
							}
							modDepts = modDepts.substring(0,
									modDepts.length - 18);
			...
			deptGrid.getStore().on('beforeload', function() {
			...
			deptGrid.getStore().on('load', function() {
				if (deptGrid.getStore().getTotalCount() != 0) {
			...
			deptGrid.on("beforeedit", function(e) {
				if (e.row == 0) {
					e.cancel = true;
				}
			});
			...
			var centerPanel = new Ext.Panel({
				...
				items : [ {
					layout : "border",
					title : "<div style='font-size:20px;'>部门管理<div>",
					items : deptGrid
				} ],
