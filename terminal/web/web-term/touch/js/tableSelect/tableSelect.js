...
$(function() {
	...
	$(window).resize(function() {
		// 餐厅选择界面高度
		$('#tableAndRegionsCmp').height(document.body.clientHeight
				- tableListHeight);
		// 点菜界面高度
		$('#orderFoodCenterCmp').height(document.body.clientHeight - 210);
		document.getElementById('foodsCmp_div_orderFood').style.height = (document.body.clientHeight - 210)
				+ 'px';
		// 沽清菜界面高度
		$('#stopSellCmp').height(document.body.clientHeight - 125);
		document.getElementById('foods4StopSellCmp').style.height = (document.body.clientHeight - 210)
				+ 'px';
		document.getElementById('divFoods4StopSellCmp').style.height = (document.body.clientHeight - 210)
				+ 'px';
		// 已点菜界面高度
		...
		// 微信预订列表
		$('#wxBookOrderListCmp_div_wxBook').height(document.body.clientHeight
				- 170);
		// 结账界面高度 & 菜品列表高度
	...
	$('#tableSelectMgr').on('pagebeforeshow', function() {
		...
		// 定时器，定时刷新餐桌选择页面数据
						(function refreshTable() {
								initTableData();
								tableRefreshTimeoutId = setTimeout(
										arguments.callee, 15 * 60 * 1000);
							})();
						}
					}
				});
			...
			$.post('../OperateBook.do', {
						...
						if (data.success) {
							if (data.root && data.root.length > 0) {
								$('#bookAmount_div_tableSelect')
										.html(data.root.length);
	...
	$('#tableSelectMgr').on('pageinit', function() {
		...
		ts.padding = new WirelessOrder.Padding({
			...
			itemLook : function(index, item) {
				...
				if (item.categoryValue == 1) {// 一般台
					...
					aliasOrName = '<font color="green">'
							+ item.name.substring(begin + 1, end) + '</font>';
				} else {
					aliasOrName = '<font color="green">' + item.categoryText
							+ '</font>';
				}
				...
				return tableCmpTemplet.format({
					...
					tempPayStatusClass : navigator.userAgent.indexOf("Firefox") >= 0
							? 'tempPayStatus4Moz'
							: 'tempPayStatus'
				});
			},
		...
		$.ajax({
			...
			success : function(data, status, xhr) {
				if (data.success) {
					...
					$('#loginStaffName_div_tableSelect')
							.html('操作人: <font color="green">'
									+ WirelessOrder.login.staffName + '</font>');
					...
					Util.msg.tip('请先登录');
					setTimeout(function() {
								location.href = 'verifyLogin.jsp?status='
										+ WirelessOrder.systemStatus.val;
							}, 2000);
				}
				Util.LM.hide();
			},
		...
		$('#idleTable_li_tableSelect').click(function() {
			...
			$('#labTableStatus').attr('table-status',
					WirelessOrder.TableList.Status.IDLE.val);
		...
		$('#busyTable_li_tableSelect').click(function() {
			$('#labTableStatus .ui-btn-text').text($(this).text());
			$('#labTableStatus').attr('table-status',
					WirelessOrder.TableList.Status.BUSY.val);
			$('#popupAllStatusCmp').popup('close');
			showTable();
		});
		...
		function dailyHandler(commitType) {
			...
			dailyPopup = new JqmPopup({
						...
						pageInit : function(self) {
							...
							// 预打
							self.find('[id=prePrint_a_daily]').click(
									function() {
										prePrint();
									});
							// 日结
			...
			function showDailyInfo(settleType) {
				...
				memberTrModel = '<tr>' + '<th>会员充值</th>'
						+ '<td class="text_right">{0}</td>'
						+ '<td class="text_right">{1}</td>'
						+ '<td class="text_right">{2}</td>' + '</tr>' + '<tr>'
						+ '<th>会员退款</th>' + '<td class="text_right">{3}</td>'
						+ '<td class="text_right">{4}</td>'
						+ '<td class="text_right">{5}</td>' + '</tr>',
				...
				trModel = '<tr>' + '<th>{0}</th>'
						...
						+ '<td class="text_right">{3}</td>' + '</tr>',

				trPayIncomeModel = '<tr>' + '<th>{0}</th>'
						+ '<td class="text_right">{1}</td>'
						+ '<td class="text_right">{2}</td>'
						+ '<td class="text_right">{3}</td>' + '</tr>';

				// 设置标题
				$('#title4DailyInfoTable').html('<font color="#f7c942">'
						+ commitType.title + '</font> -- ' + commitType.title
						+ '人 : ' + WirelessOrder.login.staffName);
				$('#confirm_a_daily .ui-btn-text').html(commitType.title);

				$.post('../QueryDailySettleByNow.do', {
							queryType : settleType.type
						}, function(jr) {
							if (jr.success) {
								...
								for (var i = 0; i < deptStat.length; i++) {
									...
									trContent += (trModel.format(
											temp.dept.name, temp.discountPrice
													.toFixed(2), temp.giftPrice
													.toFixed(2), temp.income
													.toFixed(2)));
								}

								var memberTrDate = memberTrModel
										.format(business.memberChargeByCash
														.toFixed(2),
												business.memberChargeByCard
														.toFixed(2),
												business.memberAccountCharge
														.toFixed(2),
												business.memberRefund
														.toFixed(2), 0.00,
												business.memberAccountRefund
														.toFixed(2));
								var table = '<table border="1" class="tb_base">{0}{1}</table><br><table border="1" class="tb_base">{2}{3}</table>'
										.format(memberTitle, memberTrDate,
												deptTrModel, trContent);

								// 是否有预订金额
								if (business.bookIncome > 0) {
									table += '<br><table border="1" class="tb_base"><tr><th class="table_title text_center">预订总金额:</th><th class="table_title text_center">'
											+ business.bookIncome
											+ '</th></tr></table>';
								}
								...
								$('#startDate_td_daily')
										.html(business.paramsOnDuty);
								$('#endDate_td_daily')
										.html(business.paramsOffDuty);

								$('#orderAmount_td_daily')
										.html(business.orderAmount);

								$('#eraseAmount_td_daily')
										.html(business.eraseAmount);
								$('#eraseIncome_td_daily')
										.html(business.eraseIncome.toFixed(2));

								$('#discountAmount_td_daily')
										.html(business.discountAmount);
								$('#discountIncome_td_daily')
										.html(business.discountIncome
												.toFixed(2));

								$('#giftAmount_td_daily')
										.html(business.giftAmount);
								$('#giftIncome_td_daily')
										.html(business.giftIncome.toFixed(2));

								$('#couponAmount_td_daily')
										.html(business.couponAmount);
								$('#couponIncome_td_daily')
										.html(business.couponIncome.toFixed(2));

								$('#cancelAmount_td_daily')
										.html(business.cancelAmount);
								$('#cancelIncome_td_daily')
										.html(business.cancelIncome.toFixed(2));

								$('#repaidAmount_td_daily')
										.html(business.paidAmount);
								$('#repaidIncome_td_daily')
										.html(business.paidIncome.toFixed(2));

								$('#serviceAmount_td_daily')
										.html(business.serviceAmount);
								$('#serviceIncome_td_daily')
										.html(business.serviceIncome.toFixed(2));
								...
								for (var i = 0; i < business.paymentIncomes.length; i++) {
									...
									trPayTypeContent.push(trPayIncomeModel
											.format(temp.payType, temp.amount,
								...
								trPayTypeContent.push(trPayIncomeModel.format(
										'总计', totalCount, totalShouldPay
												.toFixed(2), totalActual
												.toFixed(2)));
								$('#payIncome_table_daily')
										.html(trPayTypeContent.join(""));
							}
						});
			}
		...
		$('#pointConsume_li_tableSelect').click(function() {
			...
			setTimeout(function() {
				seajs.use(['readMember', 'issueCoupon'], function(readPopup,
						issuePopup) {
						...
						confirm : function(member) {
							if (member) {
								fastIssuePopup.close(function() {
									var issueCouponPopup = issuePopup
											.newInstance({
												title : '积分兑换',
												member : member,
												...
												confirm : function(self,
													...
													$.ajax({
														...
														data : {
															...
															memberId : member.id,
															comment : self
																	.find('[id="pointExchange_input_issue"]')
																	.val(),
															promotions : promotions
																	.join(";"),
															isIssueAndUse : self
																	.find('[id=pointExchange_check_issue]')
																	.attr('checked')
																	? true
																	: false
														},
		...
		$('#fastUse_a_tableSelect').click(function() {
			$('#frontPageMemberOperation').popup('close');
			setTimeout(function() {
				seajs.use(['readMember', 'useCoupon'], function(readPopup,
						usePopup) {
		...
		$('#memberWxBind_li_tableSelect').click(function() {
			$('#frontPageMemberOperation').popup('close');
			setTimeout(function() {
				seajs.use(['readMember', 'perfectMemberMsg'], function(
						readPopup, prefectPopup) {
						...
						confirm : function(member) {
							if (member && member.isRaw) {
								memberWxReadPopup.close(function() {
									...
									memberWxBindPopup = prefectPopup
											.newInstance({
														memberName : member.name,
		...
		$('#searchTable_a_tableSelect').click(function() {
			...
			seajs.use('askTable', function(askTable) {
						...
						if (prefectMatched) {
							askTablePopup.close(function() {
										updateTable({
													...
													alias : !prefectMatched.id
															? prefectMatched.alias
															: ''
												});
									}, 200);
						} else {
					...
					left : function() {
						...
						if (perfectMatched) {
							if (perfectMatched.statusValue == WirelessOrder.TableList.Status.BUSY.val) {
								askTablePopup.close(function() {
											$('#tableSelect_div_askTable')
													.off('keydown');
											updateTable({
														toPay : true,
														id : perfectMatched.id,
														alias : !perfectMatched.id
																? perfectMatched.alias
																: ''
													});
										}, 200);
							} else {
				...
				askTablePopup.open(function(self) {
							$('#tableSelect_div_askTable').on('keydown',
									function(event) {
			...
			seajs.use('askTable', function(askTable) {
						var askTablePopup = askTable.newInstance({
									...
									middle : function() {
										...
										var sourceAlias = $('#left_input_askTable')
												.val();
										var destAliasd = $('#tranNum_input_ask')
												.val();

										sourceTable = WirelessOrder.tables
												.getByAlias(sourceAlias);

										var destTable = WirelessOrder.tables
												.getByAlias(destAliasd);
										...
										$.post('../OperateTable.do', {
											...
											orientedPrinter : getcookie(document.domain
													+ '_printers')
										}, function(data) {
											Util.LM.hide();
											if (data.success) {
				...
				var askTablePopup = askTable.newInstance({
					...
					suffixSelect : function(suffixValue) {
						...
						$.post('../OperateTable.do', {
									...
									suffix : suffix,
									comment : $("#apartComment_input_ask")
											.val()
								}, function(result) {
									Util.LM.hide();
									if (result.success) {
								...
								of.entry({
									...
									commit : function(selectedFoods) {
										...
										for (var i = 0; i < selectedTable.length; i++) {
											...
											orderDataModel.orderFoods = selectedFoods
													.slice(0);
											orderDataModel.categoryValue = selectedTable[i].categoryValue;
											orderFoods
													.push(JSON
															.stringify(Wireless.ux
																	.commitOrderData(orderDataModel)));
										}

										Util.LM.show();
										$.post('../OperateOrderFood.do', {
													dataSource : 'multiOpenTable',
													multiTableOrderFoods : orderFoods
															.join("<li>")
												}, function(data) {
													Util.LM.hide();
													if (data.success) {
	...
	/**
	 * 根据alias或id返回table的最新状态 toPay : 是否去结账界面
	 */
				...
				if (ts.table.isBook) {
					...
					customerPopup = new NumKeyBoardPopup({
						...
						left : function() {
							...
							orderDataModel.comment = $('#inputTableOpenCommon')
									.val();

							$.post('../InsertOrder.do', {
										commitOrderData : JSON
												.stringify(Wireless.ux
														.commitOrderData(orderDataModel)),
										type : 1,
										notPrint : false
					...
					customerPopup.open(function(self) {
								...
								setTimeout(function() {
											self
													.find('[id=input_input_numKbPopup]')
													.select();
										}, 200);
	...
	/**
	 * 初始化餐桌信息，保存到tables数组中 freeTables存放空闲餐桌，busyTables存放就餐餐桌
	 */
	function initTableData() {
		...
		function showRegion() {
			...
			var html = [];
			html
					.push('<a data-role="button" data-inline="true" data-type="region" class="regionBtn">全部区域</a>');
			WirelessOrder.regions.forEach(function(e) {
				html
						.push('<a data-role="button" data-inline="true" data-type="region" class="regionBtn" region-id="'
								+ e.id + '">' + e.name + '</a>');
			});

			$('#divSelectRegionForTS').html(html.join("")).trigger('create')
					.trigger('refresh').find('.regionBtn').each(
							function(index, element) {
								element.onclick = function() {
									// 恢复所有区域按钮为未选中状态
									$('#divSelectRegionForTS .regionBtn').attr(
											'data-theme', 'c')
											.removeClass('ui-btn-up-e')
											.addClass('ui-btn-up-c')
											.removeAttr('region-selected');
									// 设置点击的区域按钮是选中状态
									$(element).attr('data-theme', 'e')
											.removeClass('ui-btn-up-c')
											.addClass('ui-btn-up-e').attr(
		...
		$.post('../QueryTable.do', null, function(data, status, xhr) {
			if (status == 'success') {
				if (data.success) {
					...
					// 设置各状态数量
					$('#idleTableAmount_label_tableSelect')
							.text(WirelessOrder.tables.getIdleAmount());
					$('#busyTableAmount_label_tableSelect')
							.text(WirelessOrder.tables.getBusyAmount());
					$('#idleTableAmount_font_tableSelect')
							.text(WirelessOrder.tables.getIdleAmount());
					$('#busyTableAmount_font_tableSelect')
							.text(WirelessOrder.tables.getBusyAmount());
					$('#tmpPaidTableAmount_font_tableSelect')
							.text(WirelessOrder.tables.getTmpPaidAmount());
