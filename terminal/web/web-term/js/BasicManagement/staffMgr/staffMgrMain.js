Ext.onReady(function(){
	var filterTypeDate = [[0, '全部'], [1, '员工姓名'], [2, '员工角色']];

	var currRowIndex = -1;
	var pageRecordCount = 30;
	...
	var roleStore = new Ext.data.Store({
		...
		listeners : {
			load : function(s, r, o){
				...
				$('#divStaff').find('.roleUpdate_a_staffMgr').each(function(index, element){
					element.onclick = function(){
						operateRole({otype:'update'});
					}
				});
				
				$('#divStaff').find('.roleDel_a_staffMgr').each(function(index, element){
					element.onclick = function(){
						operateRole({otype:'delete'});
					}
				});
			...
			rowclick : function(thiz, rowIndex, e){
				...
				if(selectChange){
					Ext.Msg.confirm('提示', '是否保存权限的修改?', function(e){
						...
						privilegeTree.loader.baseParams = {dataSource : 'roleTree', roleId : thiz.getStore().getAt(rowIndex).get('id'), branchId : Ext.getCmp('branch_combo_staffMgrMain').getValue()};
						...
						if(staffAddPwd == staffAddPwdCon){
							...
							Ext.Ajax.request({
								...
								params : {
									...
									'tele' : tele,
									'staffId' : staffId,
									'branchId' : Ext.getCmp('branch_combo_staffMgrMain').getValue()
								},
	...
	var branch_combo_staffMgrMain = new Ext.form.ComboBox({
		id : 'branch_combo_staffMgrMain',
		readOnly : false,
		forceSelection : true,
		width : 123,
		listWidth :120,
		...
		listeners : {
			...
			select : function(){
				...
				store.baseParams['branchId'] = Ext.getCmp('branch_combo_staffMgrMain').getValue();
				store.load();
				
				privilegeTree.loader.dataUrl = "../../QueryPrivilege.do";
				privilegeTree.loader.baseParams = {dataSource : 'tree', branchId : Ext.getCmp('branch_combo_staffMgrMain').getValue()};
				privilegeTree.getRootNode().reload();
	...
	var staffGrid = new Ext.grid.GridPanel({
		...
		tbar : [{
			...
			branch_combo_staffMgrMain,{xtype : 'tbtext',text : '&nbsp;&nbsp;'},
			spm_filterComb,
			{xtype : 'tbtext',text : '&nbsp;&nbsp;'},
			{
				xtype : 'textfield',
				...
				handler : function(){
					...
					store.baseParams['isPaging'] = false;
					store.baseParams['isCombo'] = false;
					store.baseParams['hasDetail'] = true;
					...
					store.on('load', function(store, records, options){
						$('#divStaff').find('.adminUpdate_a_staffMgr').each(function(index, element){
							element.onclick = function(){
								changePwdHandler($(element).attr('data-value'));
							}
						});
						
						$('#divStaff').find('.staffUpdate_a_staffMgr').each(function(index, element){
							element.onclick = function(){
								operateStaff({otype:'update'});
							}
						});
						
						$('#divStaff').find('.staffDel_a_staffMgr').each(function(index, element){
							element.onclick = function(){
								var ss = Ext.getCmp('staffGrid').getSelectionModel().getSelected();
								if(ss != null){
									Ext.Msg.confirm(
	...
	var privilegeTree = new Ext.tree.TreePanel({
		id : 'privilegeTree',   
		region : 'east',
		width : '18%',
		border : false,
...
//			return '<a href="javascript:changePwdHandler(' + rowIndex + ')"><img src="../../images/Modify.png"/>修改</a>';
			return '<a class="adminUpdate_a_staffMgr" data-value="' + rowIndex + '"><img src="../../images/Modify.png"/>修改</a>';;
		}else{
			return '<a class="staffUpdate_a_staffMgr">' + "<img src='../../images/Modify.png'/>修改</a>"
			 +"&nbsp;&nbsp;&nbsp;"
			 + '<a class="staffDel_a_staffMgr">' + "<img src='../../images/del.png'/>删除</a>";
		}
	};
	function roleOpt(value, cellmeta, record, rowIndex, columnIndex, store) {
		if(eval(record.get('categoryValue')) == 1 || eval(record.get('categoryValue') == 2)){
			...
			return ''
			+ '<a class="roleUpdate_a_staffMgr">' + "<img src='../../images/Modify.png'/>修改</a>"
			+ '&nbsp;&nbsp;&nbsp;'
			+ '<a class="roleDel_a_staffMgr">' + "<img src='../../images/del.png'/>删除</a>";
		}
	};
...
//	roleStore.baseParams['branchId'] = branch_combo_staffMgrMain.getValue();
//	roleStore.load();
});