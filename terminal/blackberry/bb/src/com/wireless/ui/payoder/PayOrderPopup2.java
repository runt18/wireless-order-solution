...
class PayOrderPopup2 extends PopupScreen{
	...
	PayOrderPopup2(Order order, PostPayOrder postPayOrder){
		...
		add(new LabelField("提交" + _orderToPay.table_id + "号台结帐信息...请稍候"));
	}
					...
					try{
						...
						if(_resp.header.type == Type.ACK){
							UiApplication.getUiApplication().invokeLater(new Runnable(){
								public void run(){
									Dialog.alert(_orderToPay.table_id + "号台结帐成功");
							...
							UiApplication.getUiApplication().invokeLater(new Runnable(){
								public void run(){
									if(_errCode == ErrorCode.TABLE_NOT_EXIST){
										Dialog.alert(_orderToPay.table_id + "号台已被删除，请与餐厅负责人确认。");
									}else if(_errCode == ErrorCode.TABLE_IDLE){
										Dialog.alert(_orderToPay.table_id + "号台的账单已结帐或删除，请与餐厅负责人确认。");
									}else if(_errCode == ErrorCode.PRINT_FAIL){
										Dialog.alert(_orderToPay.table_id + "号结帐打印未成功，请与餐厅负责人确认。");
									}else{
										Dialog.alert(_orderToPay.table_id + "号台结帐未成功，请重新结帐");
									}
									if(_postPayOrder != null){
