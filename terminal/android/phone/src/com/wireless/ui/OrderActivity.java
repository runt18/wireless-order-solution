	...
	class FoodExpandableAdapter extends SimpleExpandableListAdapter{
		...
		public View getChildView(int groupPosition, int childPosition,
			...
			if(!map.containsKey(ITEM_IS_ORI_FOOD)){
				...
				addTasteImgView.setOnClickListener(listener);
			} 
			//已点菜
			else {
...
//			if(isExpanded){
//				((ImageView)layout.findViewById(R.id.arrow)).setBackgroundResource(R.drawable.point);
//			}else{
//				((ImageView)layout.findViewById(R.id.arrow)).setBackgroundResource(R.drawable.point02);
//			}
			return layout;
		}
		...
		ExtOperDialg(final OrderFood selectedFood, final boolean isOriFood) {
			...
			if(!isOriFood){
				...
				((RelativeLayout)findViewById(R.id.r1)).setOnClickListener(new View.OnClickListener() {						
					...
					public void onClick(View arg0) {
						dismiss();
						new AskCancelAmountDialog(selectedFood,isOriFood).show();							
					}
				});
